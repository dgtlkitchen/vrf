...
// ConvertBech32Cmd converts any bech32 address to a fitting prefixed address (cosmos1... -> chain1...)
func ConvertBech32Cmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "bech32-convert [bech32 string]",
		Short: "Convert any bech32 string to the chain prefix",
		Long: `Convert any bech32 string to the chain prefix
Especially useful for converting cosmos addresses to chain addresses
Example:
	chaind bech32-convert chain1ey69r37gfxvxg62sh4r0ktpuc46pzjrm5cxnjg -p osmo
	`,
		Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
	...
	cmd.Flags().StringP(flagBech32Prefix, "p", "vrf", "Bech32 Prefix to encode to")
